...
A bad CRC in mtd1 makes the bootloader fall back to its compiled defaults. Those
defaults are now generated by `board/sigmastar/uboot-recovery-table.sh` at build
time, sized to the kernel this build produced:

```
mtdparts=NOR_FLASH:256k(boot),64k(env),<measured>k(kernel),-(rootfs)
```

Four partitions, no `data` and no `all` — it claims only what the build can
prove, because the rootfs size is not known when the bootloader compiles. The
rootfs *offset* is exact, so mtdblock3 lands on the real filesystem and the
board boots read-only on the serial console. `/init` cannot find a partition
named `data`, calls `die()`, and its EXIT trap execs `/sbin/init` anyway. Expect
no network. Re-apply `uenv.txt` to leave that state.

This replaces what used to happen, and the old behaviour is worth remembering
because it is what a board flashed with an older bootloader still does: the
compiled defaults carried the OEM table with `${rootmtd}` and a 5120k rootfs, so
a larger rootfs read as truncated — appearing to mount and failing later, which
looks like filesystem corruption rather than a partition problem. It was
slightly worse under our own bootloader than under the OEM one, because removing
the `sf probe` auto-sizing also removed the accident that rescued it.

The fix lives in `sigmastar-uboot`, which both SigmaStar families share, so it
arrived here and on Infinity6B0 in the same commit. It only takes effect on a
board reflashed with the new mtd0.
